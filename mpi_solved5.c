...
******************************************************************************/
/* Comment:
The timing is at first small, then jumps up several magnitude. This is because non-blocking send/recv keep communicating large-size message chunk, so the cache is full after hundreds of loops. Then, the speed of communication significantly decreases, since new message can only be send when old message is moved out of cache(recv).  
*/
