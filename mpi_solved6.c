...
/* Comment:
1) The count for requests is wrong: for process 0 and 1, each has 2*REPS requests, so there should be 4*REPS requests in total instead of 2*REPS
2) Requests shouldn't be overwritten, so waitall can ensure all unblocking send/recv have finished.
*/
...
MPI_Waitall(nreqs, &reqs[offset], stats);
}
T2 = MPI_Wtime();     /* end time */
MPI_Barrier(COMM);  //for clean output
